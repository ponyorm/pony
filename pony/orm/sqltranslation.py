...
class SQLTranslator(ASTTranslator):
        ...
        if t is NoneType:
            ...
            for i, item_type in enumerate(t):
                if item_type is NoneType:
                    throw(
                        TypeError, 'Expression `%s` should not contain None values' % node.src)
                param = translator.ParamMonad.new(
                    translator, item_type, (varkey, i, None))
                params.append(param)
            monad = translator.ListMonad(translator, params)
        elif isinstance(t, RawSQLType):
            monad = translator.RawSQLMonad(translator, t, varkey)
        else:
            monad = translator.ParamMonad.new(
                translator, t, (varkey, None, None))
        node.monad = monad
        monad.node = node
            ...
            if monad.aggregated:
                ...
                if monad.nogroup:
                    if isinstance(monad, ListMonad):
                        ...
                        throw(
                            TranslationError, 'Too complex aggregation, expressions cannot be combined: %s' % ast2src(node))
            return monad
    ...
    def __init__(translator, tree, extractors, vartypes, parent_translator=None, left_join=False, optimize=None):
        ...
        if not parent_translator:
            subquery = Subquery(left_join=left_join)
        else:
            subquery = Subquery(parent_translator.subquery,
                                left_join=left_join)
        ...
        for i, qual in enumerate(tree.quals):
            assign = qual.assign
            if not isinstance(assign, ast.AssName):
                throw(NotImplementedError, ast2src(assign))
            if assign.flags != 'OP_ASSIGN':
                throw(TypeError, ast2src(assign))

            name = assign.name
            if name in tablerefs:
            ...
            if monad:  # Lambda was encountered inside generator
                ...
                if not isinstance(iterable, SetType):
                ...
                if not isinstance(entity, EntityMeta):
                    throw(TranslationError, 'for %s in %s' %
                          (name, ast2src(qual.iter)))
                if i > 0:
                ...
                if parent_tableref is None:
                    throw(TranslationError,
                          "Name %r must be defined in query" % node_name)
                ...
                for j, attrname in enumerate(attr_names):
                    ...
                    entity = attr.py_type
                    if not isinstance(entity, EntityMeta):
                        throw(NotImplementedError, 'for %s in %s' %
                              (name, ast2src(qual.iter)))
                    can_affect_distinct = None
                    if attr.is_collection:
                        if not isinstance(attr, Set):
                            throw(NotImplementedError, ast2src(qual.iter))
                    ...
                    tableref = JoinedTableRef(
                        subquery, name_path, parent_tableref, attr)
            ...
            for if_ in qual.ifs:
                ...
                if isinstance(if_.monad, translator.AndMonad):
        ...
        if isinstance(monad, translator.ParamMonad):
            throw(TranslationError,
                  "External parameter '%s' cannot be used as query result" % ast2src(tree.expr))
        translator.expr_monads = monad.items if isinstance(
            monad, translator.ListMonad) else [monad]
        ...
        if isinstance(expr_type, EntityMeta):
            monad.orderby_columns = list(
                xrange(1, len(expr_type._pk_columns_) + 1))
            if monad.aggregated:
                throw(TranslationError)
            ...
            translator.expr_columns = [
                ['COLUMN', alias, column] for column in pk_columns]
            ...
            if len(expr_monads) > 1:
                translator.expr_type = tuple(
                    m.type for m in expr_monads)  # ?????
                expr_columns = []
            ...
            if translator.aggregated:
                ...
                for tr in tablerefs.values():
                    ...
                    for attr in tr.entity._pk_attrs_:
                        if (tr.name_path, attr) not in expr_set:
                            break
                    else:
                        continue
                    translator.distinct = True
            ...
            for m in expr_monads:
                ...
                if isinstance(expr_type, EntityMeta):
                    ...
                    m.orderby_columns = list(
                        xrange(offset + 1, next_offset + 1))
                    offset = next_offset
                else:
                    ...
                    m.orderby_columns = (
                        offset + 1,) if not m.disable_ordering else ()
                    offset += 1
            translator.row_layout = row_layout
            translator.col_names = [
                src for func, slice_or_offset, src in translator.row_layout]
    ...
    def shallow_copy_of_subquery_ast(translator, move_outer_conditions=True, is_not_null_checks=False):
        subquery_ast, attr_offsets = translator.construct_sql_ast(
            distinct=False, is_not_null_checks=is_not_null_checks)
    ...
    def construct_sql_ast(translator, range=None, distinct=None, aggr_func_name=None, for_update=False, nowait=False,
        ...
        if aggr_func_name:
            ...
            if isinstance(expr_type, EntityMeta):
                if aggr_func_name is not 'COUNT':
                    throw(TypeError,
                ...
                if aggr_func_name in ('SUM', 'AVG') and expr_type not in numeric_types:
                    throw(TypeError, '%r is valid for numeric attributes only' %
            ...
            if groupby_monads or (aggr_func_name == 'COUNT' and distinct
                ...
                if aggr_func_name == 'COUNT':
                    ...
                    if expr_ast[0] == 'COLUMN':
                        ...
                        outer_aggr_ast = [aggr_func_name, [
                            'COLUMN', outer_alias, column_name]]
                    else:
                        select_ast = [
                            'DISTINCT' if distinct else 'ALL'] + [['AS', expr_ast, 'expr']]
                        outer_aggr_ast = [aggr_func_name,
                                          ['COLUMN', 't', 'expr']]
                ...
                if aggr_func_name == 'COUNT':
                    if isinstance(expr_type, (tuple, EntityMeta)) and not distinct:
                        aggr_ast = ['COUNT', 'ALL']
                    else:
                        aggr_ast = ['COUNT', 'DISTINCT',
                                    translator.expr_columns[0]]
                else:
                    aggr_ast = [aggr_func_name, translator.expr_columns[0]]
            if aggr_ast:
        ...
        if is_not_null_checks:
            for monad in translator.expr_monads:
                if isinstance(monad, translator.ObjectIterMonad):
                    pass
                elif isinstance(monad, translator.AttrMonad) and not monad.attr.nullable:
                    pass
                else:
                    notnull_conditions = [['IS_NOT_NULL', column_ast]
                                          for column_ast in monad.getsql()]
                    if monad.aggregated:
        ...
        if range:
            ...
            if offset:
                limit_section.append(['VALUE', offset])
    ...
    def construct_delete_sql_ast(translator):
        ...
        if len(from_ast) == 2 and not translator.subquery.used_from_subquery:
            ...
            if len(entity._pk_columns_) == 1:
                ...
                inner_expr = expr_monad.getsql()
                outer_expr = ['ROW'] + [['COLUMN', None, column_name]
                                        for column_name in entity._pk_columns_]
            else:
                throw(NotImplementedError)
    ...
    def order_by_numbers(translator, numbers):
        ...
        for i in numbers:
            ...
            for pos in monad.orderby_columns:
                new_order.append(
                    i < 0 and ['DESC', ['VALUE', pos]] or ['VALUE', pos])
        order[:0] = new_order
        return translator
    ...
    def apply_kwfilters(translator, filterattrs):
        ...
        if not isinstance(entity, EntityMeta):
            throw(
                TypeError, 'Keyword arguments are not allowed when query result is not entity objects')
        translator = deepcopy(translator)
        expr_monad = translator.tree.expr.monad
        monads = []
        ...
        for attr, id, is_none in filterattrs:
            ...
            if is_none:
                ...
                param_monad = translator.ParamMonad.new(
                    translator, attr.py_type, (id, None, None))
    ...
    def apply_lambda(translator, filter_num, order_by, func_ast, argnames, extractors, vartypes):
        ...
        # func_ast = deepcopy(func_ast)
        func_ast = pickle.loads(pickled_func_ast)
        ...
        if order_by:
            ...
            for node in nodes:
                monad = node.monad
                if isinstance(monad, translator.AndMonad):
    ...
    def preGenExpr(translator, node):
        ...
        subtranslator = translator.__class__(
            inner_tree, translator.extractors, translator.vartypes, translator)
        return translator.QuerySetMonad(translator, subtranslator)

    def postGenExprIf(translator, node):
        monad = node.test.monad
        if monad.type is not bool:
            monad = monad.nonzero()
        return monad
    ...
    def preCompare(translator, node):
        ...
        for op, right in node.ops:
            ...
            if op == 'not in':
                translator.inside_not = not inside_not
    ...
    def preCallFunc(translator, node):
        if node.star_args is not None:
            throw(NotImplementedError, '*%s is not supported' %
                  ast2src(node.star_args))
        if node.dstar_args is not None:
            throw(NotImplementedError, '**%s is not supported' %
                  ast2src(node.dstar_args))
        func_node = node.node
        if isinstance(func_node, ast.CallFunc):
            if isinstance(func_node.node, ast.Name) and func_node.node.name == 'getattr':
                return
        if not isinstance(func_node, (ast.Name, ast.Getattr)):
            throw(NotImplementedError)
        if len(node.args) > 1:
            return
        if not node.args:
        ...
        if not isinstance(entity_monad, EntityMonad):
            throw(NotImplementedError)
        entity = entity_monad.type.item_type
        if method_monad.attrname != 'select':
            throw(TypeError)
        if len(lambda_expr.argnames) != 1:
            throw(TypeError)
        if lambda_expr.varargs:
            throw(TypeError)
        if lambda_expr.kwargs:
            throw(TypeError)
        if lambda_expr.defaults:
            throw(TypeError)
        ...
        for_expr = ast.GenExprFor(ast.AssName(
            iter_name, 'OP_ASSIGN'), name_ast, [if_expr])
        inner_expr = ast.GenExprInner(ast.Name(iter_name), [for_expr])
        subtranslator = translator.__class__(
            inner_expr, translator.extractors, translator.vartypes, translator)
        return translator.QuerySetMonad(translator, subtranslator)
    ...
    def postSubscript(translator, node):
        ...
        if len(node.subs) > 1:
            for x in node.subs:
                if isinstance(x, ast.Sliceobj):
                    throw(TypeError)
            key = translator.ListMonad(
                translator, [item.monad for item in node.subs])
        ...
        if isinstance(sub, ast.Sliceobj):
            ...
            return node.expr.monad[sub.monad]
    ...
    def postSlice(translator, node):
        ...
        if upper is not None:
            upper = upper.monad
        if isinstance(upper, NoneMonad):
            upper = None
        lower = node.lower
        if lower is not None:
            lower = lower.monad
        if isinstance(lower, NoneMonad):
            lower = None
    ...
    def postIfExp(translator, node):
        ...
        test_sql, then_sql, else_sql = test_monad.getsql(
        )[0], then_monad.getsql(), else_monad.getsql()
        if len(then_sql) == 1:
        ...
        if translator.dialect == 'PostgreSQL':
            if m1.type is bool:
                new_m1 = NumericExprMonad(
                    translator, int, ['TO_INT', m1.getsql()[0]])
                new_m1.aggregated = m1.aggregated
                m1 = new_m1
            if m2.type is bool:
                new_m2 = NumericExprMonad(
                    translator, int, ['TO_INT', m2.getsql()[0]])
    ...
    def make_join(tableref, pk_only=False):
        ...
        if not attr.is_collection:
            if not attr.columns:
                ...
                alias = subquery.make_alias(
                    tableref.var_name or rentity.__name__)
                join_cond = join_tables(
                    parent_alias, alias, left_pk_columns, reverse.columns)
            else:
                if attr.pk_offset is not None:
                    offset = attr.pk_columns_offset
                    left_columns = left_pk_columns[
                        offset:offset + len(attr.columns)]
                else:
                ...
                alias = subquery.make_alias(
                    tableref.var_name or entity.__name__)
                join_cond = join_tables(
                    parent_alias, alias, left_columns, pk_columns)
            ...
            if not tableref.joined:
                ...
                subquery.join_table(parent_alias, m2m_alias,
                                    m2m_table, m2m_join_cond)
                if pk_only:
            ...
            alias = subquery.make_alias(tableref.var_name or entity.__name__)
            join_cond = join_tables(
                m2m_alias, alias, right_m2m_columns, pk_columns)
...
class Monad(with_metaclass(MonadMeta)):
    ...
    def getattr(monad, attrname):
        try:
            ...
            if not hasattr(monad, 'call_' + attrname):
                throw(AttributeError, '%r object has no attribute %r' %
                      (type2str(monad.type), attrname))
    ...
    def count(monad):
        ...
        if monad.aggregated:
            throw(TranslationError,
                  'Aggregated functions cannot be nested. Got: {EXPR}')
        expr = monad.getsql()
        count_kind = 'DISTINCT'
        ...
        #     expr = ['ROW'] + expr
            ...
            throw(NotImplementedError,
                  ...
                  % translator.dialect)
        result = translator.ExprMonad.new(
            translator, int, ['COUNT', count_kind, expr])
    ...
    def getsql(monad, subquery=None):
        ...
        for item in monad.rawtype.items:
            if isinstance(item, basestring):
                ...
                result.append(
                    ['PARAM', (monad.varkey, i, None), param_converter])
...
def reraise_improved_typeerror(exc, func_name, orig_func_name):
    ...
    if match:
        ...
        new_msg = '%s() takes %s argument%s (%d given)' % (
            orig_func_name, what, plural, given - 1)
        exc.args = (new_msg,)
        throw(exc)
    ...
    if match:
        ...
        if not start:
            ...
            new_msg = '%s() takes at most %d argument%s (%d given)' % (
                orig_func_name, end, plural, given)
        else:
            new_msg = '%s() takes from %d to %d arguments (%d given)' % (
                orig_func_name, start, end, given)
        exc.args = (new_msg,)
        throw(exc)
...
def raise_forgot_parentheses(monad):
    ...
    throw(TranslationError, 'You seems to forgot parentheses after %s' %
          ast2src(monad.node))
...
class ListMonad(Monad):
    ...
    def contains(monad, x, not_in=False):
        ...
        if len(left_sql) == 1:
            if not_in:
                sql = ['NOT_IN', left_sql[0], [item.getsql()[0]
                                               for item in monad.items]]
            else:
                sql = ['IN', left_sql[0], [item.getsql()[0]
                                           for item in monad.items]]
        elif not_in:
            sql = sqland([sqlor([['NE', a, b] for a, b in izip(
                left_sql, item.getsql())]) for item in monad.items])
        else:
            sql = sqlor([sqland([['EQ', a, b] for a, b in izip(
                left_sql, item.getsql())]) for item in monad.items])
        return translator.BoolExprMonad(translator, sql)
...
def make_numeric_binop(op, sqlop):
    def numeric_binop(monad, monad2):
        ...
        if result_type is None:
            throw(TypeError, _binop_errmsg %
                  (type2str(monad.type), type2str(monad2.type), op))
...
def make_datetime_binop(op, sqlop):
    def datetime_binop(monad, monad2):
        ...
        delta = monad2.value if isinstance(
            monad2, TimedeltaConstMonad) else monad2.getsql()[0]
...
class StringMixin(MonadMixin):
    ...
    def __getitem__(monad, index):
        translator = monad.translator
        if isinstance(index, translator.ListMonad):
            throw(
            ...
            if start is not None and start.type is not int:
                throw(TypeError, "Invalid type of start index (expected 'int', got %r) in string slice {EXPR}" % type2str(
                    start.type))
            if stop is not None and stop.type is not int:
                throw(TypeError, "Invalid type of stop index (expected 'int', got %r) in string slice {EXPR}" % type2str(
                    stop.type))
            expr_sql = monad.getsql()[0]

            if start is None:
                start = translator.ConstMonad.new(translator, 0)

            if isinstance(start, translator.NumericConstMonad):
                if start.value < 0:
                    throw(
                        NotImplementedError, 'Negative indices are not supported in string slice {EXPR}')
        ...
        if isinstance(index, translator.NumericConstMonad):
            ...
            index_sql = ['ADD', inner_sql, ['CASE', None, [
                (['GE', inner_sql, ['VALUE', 0]], ['VALUE', 1])], ['VALUE', 0]]]
    ...
    def nonzero(monad):
        sql = monad.getsql()[0]
        translator = monad.translator
        result = translator.BoolExprMonad(
            translator, ['GT', ['LENGTH', sql], ['VALUE', 0]])
    ...
    def _like(monad, item, before=None, after=None, not_like=False):
        ...
        if isinstance(item, translator.StringConstMonad):
            ...
            if before and after:
                item_sql = ['CONCAT', ['VALUE', before],
                            item_sql, ['VALUE', after]]
            elif before:
...
class JsonMixin(object):
    ...
    def cast_from_json(monad, type):
        if type in (Json, NoneType):
            return monad
        throw(
            TypeError, 'Cannot compare whole JSON value, you need to select specific sub-item: {EXPR}')
...
class AttrMonad(Monad):
    ...
    def new(parent, attr, *args, **kwargs):
        ...
        if type in numeric_types:
            cls = translator.NumericAttrMonad
        elif type is unicode:
            cls = translator.StringAttrMonad
        elif type is date:
            cls = translator.DateAttrMonad
        elif type is time:
            cls = translator.TimeAttrMonad
        elif type is timedelta:
            cls = translator.TimedeltaAttrMonad
        elif type is datetime:
            cls = translator.DatetimeAttrMonad
        elif type is buffer:
            cls = translator.BufferAttrMonad
        elif type is UUID:
            cls = translator.UuidAttrMonad
        elif type is Json:
            cls = translator.JsonAttrMonad
        elif isinstance(type, EntityMeta):
            cls = translator.ObjectAttrMonad
        else:
            throw(NotImplementedError, type)  # pragma: no cover
...
class ObjectAttrMonad(ObjectMixin, AttrMonad):
    ...
    def __init__(monad, parent, attr):
        ...
        if monad.tableref is None:
            ...
            monad.tableref = parent_subquery.add_tableref(
...
class ParamMonad(Monad):
    ...
    def new(translator, type, paramkey):
        ...
        if type in numeric_types:
            cls = translator.NumericParamMonad
        elif type is unicode:
            cls = translator.StringParamMonad
        elif type is date:
            cls = translator.DateParamMonad
        elif type is time:
            cls = translator.TimeParamMonad
        elif type is timedelta:
            cls = translator.TimedeltaParamMonad
        elif type is datetime:
            cls = translator.DatetimeParamMonad
        elif type is buffer:
            cls = translator.BufferParamMonad
        elif type is UUID:
            cls = translator.UuidParamMonad
        elif type is Json:
            cls = translator.JsonParamMonad
        elif isinstance(type, EntityMeta):
            cls = translator.ObjectParamMonad
        else:
            throw(NotImplementedError,
                  'Parameter {EXPR} has unsupported type %r' % (type))
...
class ObjectParamMonad(ObjectMixin, ParamMonad):
    ...
    def __init__(monad, translator, entity, paramkey):
        ...
        varkey, i, j = paramkey
        assert j is None
        monad.params = tuple((varkey, i, j)
                             for j in xrange(len(entity._pk_converters_)))
...
class TimedeltaParamMonad(TimeMixin, ParamMonad):
    pass
...
class ExprMonad(Monad):
    ...
    def new(translator, type, sql):
        if type in numeric_types:
            cls = translator.NumericExprMonad
        elif type is unicode:
            cls = translator.StringExprMonad
        elif type is date:
            cls = translator.DateExprMonad
        elif type is time:
            cls = translator.TimeExprMonad
        elif type is timedelta:
            cls = translator.TimedeltaExprMonad
        elif type is datetime:
            cls = translator.DatetimeExprMonad
        elif type is Json:
            cls = translator.JsonExprMonad
        else:
            throw(NotImplementedError, type)  # pragma: no cover
...
class JsonItemMonad(JsonMixin, Monad):
    ...
    def __init__(monad, parent, key):
        ...
        if isinstance(key, (slice, EllipsisMonad)) and not translator.json_path_wildcard_syntax:
            throw(TranslationError,
                  '%s does not support wildcards in JSON path: {EXPR}' % translator.dialect)
...
class ConstMonad(Monad):
    ...
    def new(translator, value):
        ...
        if value_type in numeric_types:
            cls = translator.NumericConstMonad
        elif value_type is unicode:
            cls = translator.StringConstMonad
        elif value_type is date:
            cls = translator.DateConstMonad
        elif value_type is time:
            cls = translator.TimeConstMonad
        elif value_type is timedelta:
            cls = translator.TimedeltaConstMonad
        elif value_type is datetime:
            cls = translator.DatetimeConstMonad
        elif value_type is NoneType:
            cls = translator.NoneMonad
        elif value_type is buffer:
            cls = translator.BufferConstMonad
        elif value_type is Json:
            cls = translator.JsonConstMonad
        elif issubclass(value_type, type(Ellipsis)):
            cls = translator.EllipsisMonad
        else:
...
class BoolExprMonad(BoolMonad):
    ...
    def negate(monad):
        ...
        if negated_op is not None:
            ...
            return translator.NotMonad(translator, sql)
...
class CmpMonad(BoolMonad):
    ...
    def __init__(monad, op, left, right):
        ...
        monad.op = op
        monad.aggregated = getattr(left, 'aggregated', False) or getattr(
...
class FuncMonadMeta(MonadMeta):
    ...
    def __new__(meta, cls_name, bases, cls_dict):
        ...
        monad_cls = super(FuncMonadMeta, meta).__new__(
            meta, cls_name, bases, cls_dict)
        if func:
...
class FuncBufferMonad(FuncMonad):
    ...
    def call(monad, source, encoding=None, errors=None):
        translator = monad.translator
        if not isinstance(source, translator.StringConstMonad):
            throw(TypeError)
        source = source.value
        if encoding is not None:
            if not isinstance(encoding, translator.StringConstMonad):
                throw(TypeError)
            encoding = encoding.value
        if errors is not None:
            if not isinstance(errors, translator.StringConstMonad):
                throw(TypeError)
            errors = errors.value
        if PY2:
            if encoding and errors:
                source = source.encode(encoding, errors)
            elif encoding:
...
class FuncDecimalMonad(FuncMonad):
    ...
    def call(monad, x):
        translator = monad.translator
        if not isinstance(x, translator.StringConstMonad):
            throw(TypeError)
...
class FuncDateMonad(FuncMonad):
    ...
    def call(monad, year, month, day):
        ...
        for arg, name in izip((year, month, day), ('year', 'month', 'day')):
            if not isinstance(arg, translator.NumericMixin) or arg.type is not int:
                throw(TypeError,
    ...
    def call(monad, days=None, seconds=None, microseconds=None, milliseconds=None, minutes=None, hours=None, weeks=None):
        ...
        for arg, name in izip(args, ('days', 'seconds', 'microseconds', 'milliseconds', 'minutes', 'hours', 'weeks')):
            ...
            if not isinstance(arg, ConstMonad):
                throw(NotImplementedError)
        value = timedelta(
            *(arg.value if arg is not None else 0 for arg in args))
        return translator.ConstMonad.new(translator, value)
...
class FuncConcatMonad(FuncMonad):
    ...
    def call(monad, *args):
        if len(args) < 2:
            throw(TranslationError,
                  'concat() function requires at least two arguments')
        translator = args[0].translator
        result_ast = ['CONCAT']
        for arg in args:
            t = arg.type
            if isinstance(t, EntityMeta) or type(t) in (tuple, SetType):
                throw(TranslationError, 'Invalid argument of concat() function: %s' % ast2src(
                    arg.node))
...
class FuncCountMonad(FuncMonad):
    ...
    def call(monad, x=None):
        translator = monad.translator
        if isinstance(x, translator.StringConstMonad) and x.value == '*':
            x = None
        if x is not None:
            return x.count()
        ...
        for i, arg in enumerate(args):
            if arg.type is bool:
                args[i] = NumericExprMonad(
                    translator, int, ['TO_INT', arg.getsql()])
...
class FuncSelectMonad(FuncMonad):
    ...
    def call(monad, queryset):
        translator = monad.translator
        if not isinstance(queryset, translator.QuerySetMonad):
            throw(TypeError,
...
class FuncExistsMonad(FuncMonad):
    ...
    def call(monad, arg):
        if not isinstance(arg, monad.translator.SetMixin):
            throw(TypeError,
...
class AttrSetMonad(SetMixin, Monad):
    ...
    def cmp(monad, op, monad2):
        ...
        if type(monad2.type) is SetType \
           and are_comparable_types(monad.type.item_type, monad2.type.item_type):
               pass
    ...
    def contains(monad, item, not_in=False):
        ...
        if not translator.hint_join:
            ...
            if len(expr_list) == 1:
                subquery_ast = ['SELECT', ['ALL'] + expr_list,
                                from_ast, ['WHERE'] + conditions]
                ...
                conditions += [['EQ', expr1, expr2]
                               for expr1, expr2 in izip(item.getsql(), expr_list)]
                sql_ast = ['NOT_EXISTS' if not_in else 'EXISTS',
                           from_ast, ['WHERE'] + conditions]
            ...
            expr_ast = sqland([['EQ', expr1, expr2]
                               for expr1, expr2 in izip(expr_list, item.getsql())])
            ...
            conditions = [['EQ', ['COLUMN', alias, column], expr]
                          for column, expr in izip(columns, item.getsql())]
    ...
    def requires_distinct(monad, joined=False, for_count=False):
        ...
        if reverse.is_collection:
            ...
            if not for_count and not translator.hint_join:
                return True
            if isinstance(monad.parent, monad.translator.AttrSetMonad):
                return True
        return False
    ...
    def count(monad):
        ...
        distinct = monad.requires_distinct(
            joined=translator.hint_join, for_count=True)
        ...
        if not distinct and monad.tableref.name_path != translator.optimize:
            ...
            if monad.tableref.name_path == translator.optimize:
                ...
                make_aggr = lambda expr_list: [
                    'COUNT', 'DISTINCT' if distinct else 'ALL', ['COLUMN', alias, 'ROWID']]
            else:
                extra_grouping = True
        ...
        if sql_ast:
            ...
            sql_ast, optimized = monad._joined_subselect(
                make_aggr, extra_grouping, coalesce_to_zero=True)
        else:
            sql_ast, optimized = monad._aggregated_scalar_subselect(
    ...
    def aggregate(monad, func_name):
        ...
        translator.aggregated_subquery_paths.add(monad.tableref.name_path)
        result = translator.ExprMonad.new(
            monad.translator, result_type, sql_ast)
        if optimized:
            result.aggregated = True
        else:
            result.nogroup = True
    ...
    def make_tableref(monad, subquery):
        ...
        elif isinstance(parent, translator.AttrSetMonad):
            parent_tableref = parent.make_tableref(subquery)
        else:
            assert False  # pragma: no cover
        if attr.reverse:
    ...
    def _aggregated_scalar_subselect(monad, make_aggr, extra_grouping=False):
        ...
        if translator.optimize == monad.tableref.name_path:
            ...
            sql_ast = ['SELECT', ['AGGREGATES', make_aggr(subquery.expr_list)],
                       subquery.from_ast,
                       ['WHERE'] + subquery.outer_conditions + subquery.conditions]
    ...
    def _joined_subselect(monad, make_aggr, extra_grouping=False, coalesce_to_zero=False):
        ...
        groupby_columns = [inner_column[:]
                           for cond, outer_column, inner_column in outer_conditions]
        ...
        if extra_grouping:
            ...
            for i, column_ast in enumerate(groupby_columns + expr_list):
                ...
                if cname not in col_names:
                    ...
                    new_name = 'expr-%d' % next(
                        translator.subquery.expr_counter)
            ...
            for i, (cond, outer_column, inner_column) in enumerate(outer_conditions):
                ...
                outer_conditions[i] = [cond, outer_column,
        ...
        for cond in outer_conditions:
            cond[2][1] = alias
        translator.subquery.from_ast.append(
    ...
    def _subselect(monad):
        if monad.subquery is not None:
            return monad.subquery
        attr = monad.attr
        translator = monad.translator
        subquery = Subquery(translator.subquery)
        ...
        if not attr.reverse and not attr.is_required:
            subquery.conditions.extend(['IS_NOT_NULL', expr]
                                       for expr in subquery.expr_list)
        if subquery is not translator.subquery:
            outer_cond = subquery.from_ast[1].pop()
            if outer_cond[0] == 'AND':
                subquery.outer_conditions = outer_cond[1:]
            else:
                subquery.outer_conditions = [outer_cond]
...
class NumericSetExprMonad(SetMixin, Monad):
    ...
    def __init__(monad, op, sqlop, left, right):
        ...
        if result_type.item_type not in numeric_types:
            throw(TypeError, _binop_errmsg %
                  (type2str(left.type), type2str(right.type), op))
    ...
    def getsql(monad, subquery=None):
        ...
        if left_path.startswith(right_path):
            ...
            throw(TranslationError, 'Cartesian product detected in %s' %
                  ast2src(monad.node))
        monad.tableref = tableref
...
class QuerySetMonad(SetMixin, Monad):
    ...
    def contains(monad, item, not_in=False):
        ...
        if translator.hint_join and len(sub.subquery.from_ast[1]) == 3:
            ...
            subquery.from_ast.append([alias, 'SELECT', subquery_ast[
                                     1:], sqland(outer_conditions)])
            if not_in:
                sql_ast = sqland([['IS_NULL', ['COLUMN', alias, new_name]]
            ...
            if len(item_columns) == 1:
                subquery_ast = sub.shallow_copy_of_subquery_ast(
                    is_not_null_checks=not_in)
                sql_ast = ['NOT_IN' if not_in else 'IN',
                           item_columns[0], subquery_ast]
            elif translator.row_value_syntax:
                subquery_ast = sub.shallow_copy_of_subquery_ast(
                    is_not_null_checks=not_in)
                sql_ast = ['NOT_IN' if not_in else 'IN',
                           ['ROW'] + item_columns, subquery_ast]
            else:
                subquery_ast = sub.shallow_copy_of_subquery_ast()
                select_ast, from_ast, where_ast = subquery_ast[1:4]
                in_conditions = [['EQ', expr1, expr2]
                                 for expr1, expr2 in izip(item_columns, select_ast[1:])]
    ...
    def count(monad):
        ...
        if isinstance(expr_type, (tuple, EntityMeta)):
            if not sub.distinct:
                ...
                select_ast = ['AGGREGATES', [
                    'COUNT', 'DISTINCT'] + sub.expr_columns]
            elif translator.dialect == 'Oracle':
            ...
            select_ast = ['AGGREGATES', [
                'COUNT', 'DISTINCT', sub.expr_columns[0]]]
        else:
            throw(NotImplementedError)  # pragma: no cover
