    ...
    def get_objects_to_create(table, created_tables=None):
        ...
        for column in table.column_list:
            if column.is_pk == 'auto':
                sequence_name = column.converter.attr.kwargs.get(
                    'sequence_name')
...
class OraTrigger(DBObject):
    ...
    def exists(trigger, provider, connection, case_sensitive=True):
        ...
        cursor = connection.cursor()
        cursor.execute(
            sql, dict(tbn=table_name, trn=trigger.name, o=owner_name))
        row = cursor.fetchone()
        return row[0] if row is not None else None
...
class OraBuilder(SQLBuilder):
    ...
    def INSERT(builder, table_name, columns, values, returning=None):
        ...
        if returning is not None:
            result.extend(
                (' RETURNING ', builder.quote_name(returning), ' INTO :new_id'))
        return result
        ...
        try:
            ...
            if not limit:
                ...
                if limit[0] == 'VALUE' and offset[0] == 'VALUE' \
                    ...
                    result.extend(
                        ('WHERE ROWNUM <= ', builder(total_limit), '\n'))
                else:
                    result.extend(('WHERE ROWNUM <= ', builder(
                        limit), ' + ', builder(offset), '\n'))
                result.extend((indent, ') t WHERE "row-num" > ',
                               builder(offset), '\n'))
            if builder.indent:
                indent = builder.indent_spaces * builder.indent
                return '(\n', result, indent + ')'
    ...
    def JSON_CONTAINS(builder, expr, path, key):
        ...
        if json_item_re.match(key[1]):
            ...
            if has_wildcards:
                ...
                pattern = r'%s%s%s' % (
                    wrapper_list_start, pattern, wrapper_list_end)
...
class OraIntConverter(dbapiprovider.IntConverter):
    signed_types = {None: 'NUMBER(38)', 8: 'NUMBER(3)', 16: 'NUMBER(5)',
                    24: 'NUMBER(7)', 32: 'NUMBER(10)', 64: 'NUMBER(19)'}
    unsigned_types = {None: 'NUMBER(38)', 8: 'NUMBER(3)', 16: 'NUMBER(5)',
                      24: 'NUMBER(8)', 32: 'NUMBER(10)', 64: 'NUMBER(20)'}
    ...
    def init(self, kwargs):
        ...
        if sequence_name is not None and not (self.attr.auto and self.attr.is_pk):
            throw(
                TypeError, "Parameter 'sequence_name' can be used only for PrimaryKey attributes with auto=True")
...
class OraTimeConverter(dbapiprovider.TimeConverter):
    ...
    def __init__(converter, provider, py_type, attr=None):
        dbapiprovider.TimeConverter.__init__(
            converter, provider, py_type, attr)
        if attr is not None and converter.precision > 0:
            # cx_Oracle 5.1.3 corrupts microseconds for values of DAY TO SECOND
            # type
...
class OraTimedeltaConverter(dbapiprovider.TimedeltaConverter):
    ...
    def __init__(converter, provider, py_type, attr=None):
        dbapiprovider.TimedeltaConverter.__init__(
            converter, provider, py_type, attr)
        if attr is not None and converter.precision > 0:
            # cx_Oracle 5.1.3 corrupts microseconds for values of DAY TO SECOND
            # type
...
class OraJsonConverter(dbapiprovider.JsonConverter):
    ...
    # CLOBs cannot be compared with strings, and TO_CHAR(CLOB) returns first
    # 4000 chars only
    optimistic = False
...
class OraProvider(DBAPIProvider):
    ...
    def execute(provider, cursor, sql, arguments=None, returning_id=False):
        if type(arguments) is list:
            ...
            if returning_id:
                var = cursor.var(cx_Oracle.STRING, 40,
                                 cursor.arraysize, outconverter=int)
    ...
    def index_exists(provider, connection, table_name, index_name, case_sensitive=True):
        ...
        if not isinstance(index_name, basestring):
            throw(NotImplementedError)
        if case_sensitive:
            sql = 'SELECT index_name FROM all_indexes WHERE owner = :o ' \
    ...
    def fk_exists(provider, connection, table_name, fk_name, case_sensitive=True):
        owner_name, table_name = provider.split_table_name(table_name)
        if not isinstance(fk_name, basestring):
            throw(NotImplementedError)
        if case_sensitive:
