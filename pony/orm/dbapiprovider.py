...
class DBAPIProvider(object):
    ...
    def get_default_fk_name(provider, child_table_name, parent_table_name, child_column_names):
        fk_name = 'fk_%s__%s' % (
            child_table_name, '__'.join(child_column_names))
...
class Converter(object):
    ...
    def __init__(converter, provider, py_type, attr=None):
        ...
        for option in kwargs:
            throw(TypeError, 'Attribute %s has unknown option %r' %
                  (attr, option))
    ...
    def get_sql_type(converter, attr=None):
        ...
        if attr is not None and attr.reverse and not attr.is_collection:
            ...
            assert rpk_converters is not None and len(
                attr.converters) == len(rpk_converters)
...
class IntConverter(Converter):
    signed_types = {None: 'INTEGER', 8: 'TINYINT', 16: 'SMALLINT',
                    24: 'MEDIUMINT', 32: 'INTEGER', 64: 'BIGINT'}
    unsigned_types = None
    ...
    def init(converter, kwargs):
        ...
        if min_val is not None and not isinstance(min_val, int_types):
            throw(TypeError, "'min' argument for attribute %s must be int. Got: %r" % (
                attr, min_val))

        max_val = kwargs.pop('max', None)
        if max_val is not None and not isinstance(max_val, int_types):
            throw(TypeError, "'max' argument for attribute %s must be int. Got: %r" % (
                attr, max_val))
        ...
        if size is None:
            ...
            throw(TypeError, "'size' option for attribute %s must be of int type. Got: %r" % (
                attr, size))
        ...
        if unsigned is not None and not isinstance(unsigned, bool):
            throw(TypeError, "'unsigned' option for attribute %s must be of bool type. Got: %r" % (
                attr, unsigned))
        ...
        lowest = highest = None
        if size:
            highest = highest = 2 ** size - \
                1 if unsigned else 2 ** (size - 1) - 1
    ...
    def validate(converter, val):
        if isinstance(val, int_types):
            ...
            throw(TypeError, 'Value type for attribute %s must be int. Got: %r' % (
    ...
    def init(converter, kwargs):
        ...
        if min_val is not None:
            try:
                ...
                throw(TypeError, "Invalid value for 'min' argument for attribute %s: %r" % (
                    converter.attr, min_val))
        max_val = kwargs.pop('max', None)
        if max_val is not None:
            try:
                max_val = float(max_val)
            except ValueError:
                throw(TypeError, "Invalid value for 'max' argument for attribute %s: %r" % (
                    converter.attr, max_val))
        converter.min_val = min_val
        converter.max_val = max_val
        converter.tolerance = kwargs.pop(
            'tolerance', converter.default_tolerance)

    def validate(converter, val):
        try:
            val = float(val)
        except ValueError:
            throw(TypeError, 'Invalid value for attribute %s: %r' %
                  (converter.attr, val))
        if converter.min_val and val < converter.min_val:
            throw(ValueError, 'Value %r of attr %s is less than the minimum allowed value %r'
                  % (val, converter.attr, converter.min_val))
...
class DecimalConverter(Converter):
    ...
    def init(converter, kwargs):
        ...
        args = attr.args
        if len(args) > 2:
            throw(TypeError, 'Too many positional parameters for Decimal '
        ...
        if not isinstance(precision, int_types):
            throw(TypeError, "'precision' positional argument for attribute %s must be int. Got: %r" % (
                attr, precision))
        if precision <= 0:
            throw(TypeError,
        ...
        if not isinstance(scale, int_types):
            throw(TypeError, "'scale' positional argument for attribute %s must be int. Got: %r" % (
                attr, scale))
        if scale <= 0:
            throw(TypeError,
...
class BlobConverter(Converter):
    ...
    def validate(converter, val):
        ...
        if isinstance(val, str):
            return buffer(val)
        throw(TypeError, "Attribute %r: expected type is 'buffer'. Got: %r" %
              (converter.attr, type(val)))
...
class DateConverter(Converter):
    ...
    def validate(converter, val):
        ...
        if isinstance(val, basestring):
            return str2date(val)
        throw(TypeError, "Attribute %r: expected type is 'date'. Got: %r" %
              (converter.attr, val))
...
class ConverterWithMicroseconds(Converter):
    ...
    def init(converter, kwargs):
        ...
        args = attr.args
        if len(args) > 1:
            throw(TypeError, 'Too many positional parameters for attribute %s. '
        ...
        if args:
            ...
            precision = kwargs.pop(
                'precision', provider.default_time_precision)
...
class TimeConverter(ConverterWithMicroseconds):
    ...
    def validate(converter, val):
        if isinstance(val, time):
            ...
            throw(TypeError, "Attribute %r: expected type is 'time'. Got: %r" %
                  (converter.attr, val))
        mcs = converter.round_microseconds_to_precision(
            val.microsecond, converter.precision)
        if mcs is not None:
...
class TimedeltaConverter(ConverterWithMicroseconds):
    ...
    def validate(converter, val):
        if isinstance(val, timedelta):
            ...
            throw(TypeError, "Attribute %r: expected type is 'timedelta'. Got: %r" % (
                converter.attr, val))
        mcs = converter.round_microseconds_to_precision(
            val.microseconds, converter.precision)
        if mcs is not None:
...
class DatetimeConverter(ConverterWithMicroseconds):
    ...
    def validate(converter, val):
        if isinstance(val, datetime):
            ...
            throw(TypeError, "Attribute %r: expected type is 'datetime'. Got: %r" % (
                converter.attr, val))
        mcs = converter.round_microseconds_to_precision(
            val.microsecond, converter.precision)
        if mcs is not None:
