...
def log_sql(sql, arguments=None):
    if type(arguments) is list:
        sql = 'EXECUTEMANY (%d)\n%s' % (len(arguments), sql)
    if logging.root.handlers:
        # arguments can hold sensitive information
...
class DBSessionContextManager(object):
    ...
    def __init__(db_session, retry=0, immediate=False, ddl=False, serializable=False, strict=False,
        ...
        if retry is not 0:
            ...
            if ddl:
                throw(
                    TypeError, "'ddl' and 'retry' parameters of db_session cannot be used together")
    ...
    def __exit__(db_session, exc_type=None, exc=None, tb=None):
        ...
        try:
            if exc_type is None:
                ...
                can_commit = issubclass(exc_type, tuple(
                    db_session.allowed_exceptions))
            else:
        ...
        def new_func(func, *args, **kwargs):
            if db_session.ddl and local.db_context_counter:
                if isinstance(func, types.FunctionType):
                    func = func.__name__ + '()'
                throw(TransactionError,
                      '%s cannot be called inside of db_session' % func)
            exc = tb = None
            try:
    ...
    def _wrap_generator_function(db_session, gen_func):
        for option in ('ddl', 'retry', 'serializable'):
            if getattr(db_session, option, None):
                throw(TypeError,
            ...
            try:
                while True:
                    try:
                        ...
                        output = wrapped_interact(
                            iterator, exc_info=sys.exc_info())
                    else:
...
def with_transaction(*args, **kwargs):
    deprecated(
        3, "@with_transaction decorator is deprecated, use @db_session decorator instead")
...
class Database(object):
    ...
    def __init__(self, *args, **kwargs):
        # argument 'self' cannot be named 'database', because 'database' can be
        # in kwargs
        self.priority = 0
        self._insert_cache = {}
    ...
    def _bind(self, *args, **kwargs):
        ...
        if self.provider is not None:
            throw(TypeError, 'Database object was already bound to %s provider' %
                  self.provider.dialect)
        if not args:
            throw(
                TypeError, 'Database provider should be specified as a first positional argument')
    ...
    def _update_local_stat(database, sql, query_start_time):
        ...
        stat = stats.get(sql)
        if stat is not None:
            stat.query_executed(query_start_time)
        else:
            stats[sql] = QueryStat(sql, query_start_time)
    ...
    def global_stats_lock(database):
        deprecated(
            3, "global_stats_lock is deprecated, just use global_stats property without any locking")
    ...
    def disconnect(database):
        ...
        if local.db_context_counter:
            throw(TransactionError,
                  'disconnect() cannot be called inside of db_sesison')
    ...
    def insert(database, table_name, returning=None, **kwargs):
        ...
        if database.provider is None:
            throw(MappingError, 'Database object is not bound with a provider yet')
        # keys are not sorted deliberately!!
        query_key = (table_name,) + tuple(kwargs)
        ...
        arguments = adapter(values_list(kwargs))
        if returning is not None:
    ...
    def _exec_sql(database, sql, arguments=None, returning_id=False, start_transaction=False):
        ...
        if not returning_id:
            return cursor
        if PY2 and type(new_id) is long:
            new_id = int(new_id)
        return new_id
    ...
    def generate_mapping(database, filename=None, check_tables=True, create_tables=False):
        ...
        if provider is None:
            throw(MappingError, 'Database object is not bound with a provider yet')
        if database.schema:
            throw(MappingError, 'Mapping was already generated')
        if filename is not None:
            throw(NotImplementedError)
        schema = database.schema = provider.dbschema_cls(provider)
        entities = list(
            sorted(database.entities.values(), key=attrgetter('_id_')))
        ...
        for entity in entities:
                ...
                if attr.is_collection:
                    ...
                    if attr.table:
                        if not reverse.table:
                            ...
                            throw(MappingError, "Parameter 'table' for %s and %s do not match" % (
                                attr, reverse))
                        table_name = attr.table
                    elif reverse.table:
                        table_name = attr.table = reverse.table
                    else:
                        table_name = provider.get_default_m2m_table_name(
                            attr, reverse)
                    ...
                    if m2m_table is not None:
                        if not attr.table:
                            ...
                            throw(
                                MappingError, "Table name '%s' is already in use" % table_name)
                        else:
                            throw(NotImplementedError)
                    ...
                    m2m_table.add_index(None, tuple(
                        m2m_table.column_list), is_pk=True)
                    ...
                    if attr.is_required:
                        ...
                        if attr.nullable is False:
                            throw(
                                TypeError, 'Optional attribute with non-string type %s must be nullable' % attr)
                        attr.nullable = True
                    elif entity._database_.provider.dialect == 'Oracle':
                        if attr.nullable is False:
                            throw(ERDiagramError,
                    ...
                    if len(columns) == 1:
                        ...
                        for (column_name, converter) in izip(columns, attr.converters):
                            table.add_column(
                                column_name, converter.get_sql_type(), converter, not attr.nullable)
                    else:
            ...
            if not table.pk_index:
                if len(entity._pk_columns_) == 1 and entity._pk_attrs_[0].auto:
                    ...
                    is_pk = True
                table.add_index(None, get_columns(
                    table, entity._pk_columns_), is_pk)
            for index in entity._indexes_:
                ...
                table.add_index(index_name, get_columns(
                    table, column_names), is_unique=index.is_unique)
            columns = []
            columns_without_pk = []
            converters = []
            ...
            for attr in entity._new_attrs_:
                if attr.is_collection:
                    ...
                    m2m_table.add_foreign_key(
                        None, child_columns, table, parent_columns, attr.index)
                    if attr.symmetric:
                        child_columns = get_columns(
                            m2m_table, attr.reverse_columns)
                        m2m_table.add_foreign_key(
                            None, child_columns, table, parent_columns)
                elif attr.reverse and attr.columns:
                    rentity = attr.reverse.entity
                    parent_table = schema.tables[rentity._table_]
                    parent_columns = get_columns(
                        parent_table, rentity._pk_columns_)
                    child_columns = get_columns(table, attr.columns)
                    table.add_foreign_key(
                        None, child_columns, parent_table, parent_columns, attr.index)
                elif attr.index and attr.columns:
                    columns = tuple(
                        imap(table.column_dict.__getitem__, attr.columns))
                    table.add_index(attr.index, columns,
                                    is_unique=attr.is_unique)
            entity._initialize_bits_()

        if create_tables:
            database.create_tables(check_tables)
        elif check_tables:
    ...
    def drop_table(database, table_name, if_exists=False, with_all_data=False):
        table_name = database._get_table_name(table_name)
        database._drop_tables([table_name], if_exists,
                              with_all_data, try_normalized=True)
    ...
    def _get_table_name(database, table_name):
        ...
        # table_name = templating.plainstr(table_name)
    ...
    def _drop_tables(database, table_names, if_exists, with_all_data, try_normalized=False):
        ...
        for table_name in existed_tables:
            if debug:
                log_orm('DROPPING TABLE %s' % table_name)
        ...
        for entity in sorted(database.entities.values(), key=attrgetter('_id_')):
            ...
            for attr in entity._new_attrs_:
                if not can_view(user, attr):
                    continue
                d = dict(name=attr.name, type=attr.py_type.__name__,
                ...
                attrs.append(d)
            d = dict(name=entity.__name__, newAttrs=attrs, pkAttrs=[
                     attr.name for attr in entity._pk_attrs_])
            ...
            if entity._composite_keys_:
                d['compositeKeys'] = [[attr.name for attr in attrs]
                                      for attrs in entity._composite_keys_]
            result.append(d)
        return result

    def _get_schema_json(database):
        schema_json = json.dumps(database._get_schema_dict(
        ), default=basic_converter, sort_keys=True)
    ...
    def to_json(database, data, include=(), exclude=(), converter=None, with_schema=True, schema_hash=None):
        ...
        if caches:
            ...
            if cache.database is not database:
                throw(TransactionError,
                      'An object does not belong to specified database')
    ...
    def from_json(database, changes, observer=None):
        ...
        def user_has_no_rights_to(operation, x):
            user_groups = get_user_groups(user)
            s = 'attribute %s' % x if isinstance(
                x, Attribute) else 'object %s' % x
            throw(PermissionError, 'The current user %s which belongs to groups %s '
            ...
            for name, val in diff.items():
                if name not in ('class', '_pk_', '_id_', '_status_'):
                    ...
                    if not attr.is_collection:
                        if type(val) is dict:
                            if 'old' in val:
                                oldvals[attr.name] = oldadict[
                                    attr] = attr.validate(id2obj(attr, val['old']))
                            if 'new' in val:
                                newvals[attr.name] = newadict[
                                    attr] = attr.validate(id2obj(attr, val['new']))
                        else:
                            newvals[attr.name] = newadict[
                                attr] = attr.validate(id2obj(attr, val))
            oid = diff['_id_']
...
def _split_names(typename, names):
    ...
    if isinstance(names, basestring):
        ...
        try:
            ...
            throw(TypeError, '%s name should be string. Got: %s' %
                  (typename, names))
        names = []
    ...
    for name in names:
        if not is_ident(name):
            throw(TypeError, '%s name should be identifier. Got: %s' %
                  (typename, name))
    return set(names)
...
def has_perm(user, perm, x):
    if isinstance(x, EntityMeta):
        ...
        throw(TypeError, "The third parameter of 'has_perm' function should be entity class, entity instance "
...
class SessionCache(object):
    ...
    def __init__(cache, database):
        ...
        cache.perm_cache = defaultdict(lambda: defaultdict(dict))
    ...
    def connect(cache):
        ...
        try:
            ...
            provider.set_transaction_mode(connection, cache)
        except:
    ...
    def prepare_connection_for_query_execution(cache):
        ...
        if db_session is not None and cache.db_session is None:
            ...
            assert cache.db_session is db_session, (
                cache.db_session, db_session)
        connection = cache.connection
        if connection is None:
            ...
            try:
                ...
                connection = cache.reconnect(e)
        if not cache.noflush_counter and cache.modified:
            cache.flush()
        return connection
    ...
    def close(cache, rollback=True):
        ...
        if rollback:
            try:
                provider.rollback(connection, cache)
            except:
                provider.drop(connection, cache)
                raise
    ...
    def flush(cache):
        if cache.noflush_counter:
            return
        assert cache.is_alive
        assert not cache.saved_objects
        if not cache.immediate:
            cache.immediate = True
        for i in xrange(50):
    ...
    def update_simple_index(cache, obj, attr, old_val, new_val, undo):
        ...
        if new_val is not None:
            ...
            if obj2 is not obj:
                throw(CacheIndexError, 'Cannot update %s.%s: %s with key %s already exists'
    ...
    def db_update_simple_index(cache, obj, attr, old_dbval, new_dbval):
        ...
        if new_dbval is not None:
            ...
            if obj2 is not obj:
               ...
               # attribute which was created or updated lately clashes with one
               # stored in database
...
class Attribute(object):
    ...
    def __init__(attr, py_type, *args, **kwargs):
        ...
        attr.is_string = type(py_type) is type and issubclass(
            py_type, basestring)
        attr.is_collection = isinstance(attr, Collection)
        attr.is_relation = isinstance(
            attr.py_type, (EntityMeta, basestring, types.FunctionType))
        ...
        if not attr.reverse:
            ...
            throw(TypeError, "Value of 'reverse' option must be name of reverse attribute). Got: %r" %
                  attr.reverse)
        elif not attr.is_relation:
            throw(TypeError, 'Reverse option cannot be set for this type: %r' %
                  attr.py_type)
        ...
        if attr.column is not None:
            if attr.columns is not None:
                throw(
                    TypeError, "Parameters 'column' and 'columns' cannot be specified simultaneously")
            if not isinstance(attr.column, basestring):
                throw(
                    TypeError, "Parameter 'column' must be a string. Got: %r" % attr.column)
            attr.columns = [attr.column]
        elif attr.columns is not None:
            if not isinstance(attr.columns, (tuple, list)):
                throw(
                    TypeError, "Parameter 'columns' must be a list. Got: %r'" % attr.columns)
            for column in attr.columns:
                if not isinstance(column, basestring):
                    throw(
                        TypeError, "Items of parameter 'columns' must be strings. Got: %r" % attr.columns)
            if len(attr.columns) == 1:
                attr.column = attr.columns[0]
        else:
            attr.columns = []
    ...
    def _init_(attr, entity, name):
        ...
        if 'default' in attr.kwargs:
            ...
            elif attr.default is None and not attr.nullable:
                throw(TypeError,
        ...
        if attr.py_check is not None and not callable(attr.py_check):
            throw(
                TypeError, "'py_check' parameter of %s attribute should be callable" % attr)

        # composite keys will be checked later inside EntityMeta.__init__
        if attr.py_type == float:
            if attr.is_pk:
                throw(
                    TypeError, 'PrimaryKey attribute %s cannot be of type float' % attr)
            elif attr.is_unique:
    ...
    def validate(attr, val, obj=None, entity=None, from_db=False):
        if val is None:
            if not attr.nullable and not from_db and not attr.is_required:
                # for required attribute the exception will be thrown later
        ...
        if not reverse:
            if isinstance(val, Entity):
                throw(TypeError, 'Attribute %s must be of %s type. Got: %s'
        ...
        if attr.py_check is not None and not attr.py_check(val):
            throw(ValueError, 'Check for attribute %s failed. Value: %s' %
                  (attr, truncate_repr(val)))
        return val
    ...
    def load(attr, obj):
        ...
        if attr.lazy:
            ...
            if not attr.lazy_sql_cache:
                select_list = ['ALL'] + [['COLUMN', None, column]
                                         for column in attr.columns]
                ...
                sql_ast = ['SELECT', select_list,
                           from_list, ['WHERE'] + criteria_list]
    ...
    def get(attr, obj):
        ...
        vals = obj._vals_
        if vals is None:
            throw_db_session_is_over(obj, attr)
    ...
    def __set__(attr, obj, new_val, undo_funcs=None):
        ...
        if attr.pk_offset is not None:
            ...
            if pkval is None:
                ...
                if new_val == pkval[attr.pk_offset]:
                    return
            elif new_val == pkval:
                return
        ...
        with cache.flush_disabled():
            ...
            def undo_func():
                ...
                if objects_to_save_needs_undo:
                    ...
                    assert obj2 is obj and obj._save_pos_ == len(
                        objects_to_save)
                    obj._save_pos_ = None
    ...
    def db_set(attr, obj, new_dbval, is_reverse_call=False):
        ...
        if obj._rbits_ & bit:
            ...
            if new_dbval is NOT_LOADED:
                ...
                diff = ' (was: %s, now: %s)' % (old_dbval, new_dbval)
        ...
        if not reverse:
            ...
            attr.db_update_reverse(obj, old_dbval, new_dbval)
    ...
    def get_columns(attr):
        ...
        if not reverse:  # attr is not part of relationship
            if not attr.columns:
                attr.columns = provider.get_default_column_names(attr)
            elif len(attr.columns) > 1:
                throw(MappingError, "Too many columns were specified for %s" % attr)
            ...
            def generate_columns():
                ...
                if not attr.columns:
                    attr.columns = provider.get_default_column_names(
                        attr, reverse_pk_columns)
                elif len(attr.columns) != len(reverse_pk_columns):
                    throw(MappingError,
                          'Invalid number of columns specified for %s' % attr)
                attr.col_paths = ['-'.join((attr.name, paths))
                                  for paths in reverse_pk_col_paths]
                attr.converters = []
            ...
            # one-to-one:
                ...
                pass
            elif attr.entity.__name__ > reverse.entity.__name__:
                pass
            else:
                generate_columns()
    ...
    def describe(attr):
        ...
        options = []
        if attr.args:
            options.append(', '.join(imap(str, attr.args)))
        if attr.auto:
            options.append('auto=True')
        if not isinstance(attr, PrimaryKey) and attr.is_unique:
            options.append('unique=True')
        if attr.default is not None:
            options.append('default=%r' % attr.default)
        if not options:
            options = ''
        else:
    ...
    def validate(attr, val, obj=None, entity=None, from_db=False):
        ...
        if val == '' or (val is None and not (attr.auto or attr.is_volatile or attr.sql_default)):
            throw(ValueError, 'Attribute %s is required' %
                  (attr if obj is None else '%r.%s' % (obj, attr.name)))
        return val
...
class Discriminator(Required):
    ...
    def process_entity_inheritance(attr, entity):
        ...
        if discr_value is not None:
            try:
                entity._discriminator_ = discr_value = attr.validate(
                    discr_value)
            except ValueError:
                throw(TypeError,
            ...
            throw(TypeError, "Discriminator value for entity %s "
    ...
    def _init_(index, entity):
        ...
        for i, attr in enumerate(attrs):
            if not isinstance(attr, Attribute):
                ...
                throw(TypeError, '%s() arguments must be attributes. Got: %r' %
                      (func_name, attr))
            if index.is_unique:
            ...
            if index.is_pk and attr.is_volatile:
                throw(
                    TypeError, 'Volatile attribute %s cannot be part of primary key' % attr)
            if not attr.is_required:
                if attr.nullable is False:
                    throw(
                        TypeError, 'Optional attribute %s must be nullable, because it is part of composite key' % attr)
                attr.nullable = True
...
class PrimaryKey(Required):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not attrs:
            ...
            throw(TypeError, 'PrimaryKey got invalid arguments: %r %r' %
                  (args, kwargs))
        elif len(attrs) == 1:
            attr = attrs[0]
            attr_name = 'something'
            ...
            type_str = py_type.__name__ if type(
                py_type) is type else repr(py_type)
...
class Collection(Attribute):
    ...
    def __init__(attr, py_type, *args, **kwargs):
        if attr.__class__ is Collection:
        ...
        if table is not None and not isinstance(table, basestring):
            ...
            for name_part in table:
                if not isinstance(name_part, basestring):
                    throw(
                        TypeError, 'Each part of table name must be a string. Got: %r' % name_part)
            table = tuple(table)
        attr.table = table
        Attribute.__init__(attr, py_type, *args, **kwargs)
        if attr.auto:
        ...
        if attr.reverse_column is not None:
            if attr.reverse_columns is not None and attr.reverse_columns != [attr.reverse_column]:
                throw(
                    TypeError, "Parameters 'reverse_column' and 'reverse_columns' cannot be specified simultaneously")
            if not isinstance(attr.reverse_column, basestring):
                throw(TypeError, "Parameter 'reverse_column' must be a string. Got: %r" %
                      attr.reverse_column)
            ...
            if not isinstance(attr.reverse_columns, (tuple, list)):
                throw(TypeError, "Parameter 'reverse_columns' must be a list. Got: %r" %
                      attr.reverse_columns)
            for reverse_column in attr.reverse_columns:
                if not isinstance(reverse_column, basestring):
                    throw(TypeError, "Parameter 'reverse_columns' must be a list of strings. Got: %r" %
                          attr.reverse_columns)
            if len(attr.reverse_columns) == 1:
                attr.reverse_column = attr.reverse_columns[0]
        else:
            attr.reverse_columns = []

        attr.nplus1_threshold = kwargs.pop('nplus1_threshold', 1)
        for option in attr.kwargs:
            throw(TypeError, 'Unknown option %r' % option)
    ...
    def _init_(attr, entity, name):
        ...
        attr.symmetric = (
            attr.py_type == entity.__name__ and attr.reverse == name)
        if not attr.symmetric and attr.reverse_columns:
            throw(TypeError,
...
def construct_batchload_criteria_list(alias, columns, converters, batch_size, row_value_syntax, start=0, from_seeds=True):
    ...
    if len(columns) == 1:
        ...
        param_list = [param(i + start, 0, converter)
...
class Set(Collection):
    ...
    def validate(attr, val, obj=None, entity=None, from_db=False):
        ...
        reverse = attr.reverse
        if val is None:
            throw(ValueError, 'A single %(cls)s instance or %(cls)s iterable is expected. '
    ...
    def load(attr, obj, items=None):
        ...
        if cache is not database._get_cache():
            throw(TransactionError,
                  "Transaction of object %s belongs to different thread")
        ...
        if items and (attr.lazy or not setdata):
            ...
            arguments = adapter(items)
            cursor = database._exec_sql(sql, arguments)
            loaded_items = {rentity._get_by_raw_pkval_(
        ...
        if not reverse.is_collection:
            ...
            if len(objects) > 1:
                ...
                d[obj] = {rentity._get_by_raw_pkval_(
                    row) for row in cursor.fetchall()}
    ...
    def construct_sql_m2m(attr, batch_size=1, items_count=0):
        ...
        if cached_sql is not None:
            return cached_sql
        reverse = attr.reverse
        assert reverse is not None and reverse.is_collection and issubclass(
            reverse.py_type, Entity)
        table_name = attr.table
        ...
        sql, adapter = attr.cached_load_sql[
            cache_key] = database._ast2sql(sql_ast)
        return sql, adapter
    ...
    def __set__(attr, obj, new_items, undo_funcs=None):
        ...
        if to_remove:
            if added:
                (to_remove, setdata.added) = (
                    to_remove - added, added - to_remove)
            if removed:
                removed |= to_remove
            else:
        ...
        if attr.symmetric or entity is reverse.entity:
            ...
            if attr.symmetric:
                if not attr.reverse_columns:
                    attr.reverse_columns = [
                        column + '_2' for column in attr.columns]
                elif len(attr.reverse_columns) != pk_length:
                    throw(
                        MappingError, "Invalid number of reverse columns for symmetric attribute %s" % attr)
    ...
    def remove_m2m(attr, removed):
        ...
        if cached_sql is None:
            ...
            for i, (column, converter) in enumerate(izip(columns, converters)):
                where_list.append([converter.EQ, ['COLUMN', None, column], [
    ...
    def add_m2m(attr, added):
        ...
        if cached_sql is None:
            ...
            if attr.symmetric:
                ...
                columns = reverse.columns + attr.columns
                converters = reverse.converters + attr.converters
            params = [['PARAM', (i, None, None), converter]
...
class SetInstance(object):
    ...
    @cut_traceback
    def __str__(wrapper):
        if not wrapper._obj_._session_cache_.is_alive:
            content = '...'
        else:
    ...
    def __contains__(wrapper, item):
        ...
        if not reverse.is_collection:
            obj2 = item._vals_[
                reverse] if reverse in item._vals_ else reverse.load(item)
            wbits = item._wbits_
    ...
    def add(wrapper, new_items):
        ...
        removed = setdata.removed
        if removed:
            (new_items, setdata.removed) = (
                new_items - removed, removed - new_items)
        if added:
            added |= new_items
        else:
...
class Multiset(object):
    ...
    def __init__(multiset, obj, attrnames, items):
        ...
        if type(items) is dict:
            multiset._items_ = items
        else:
            multiset._items_ = utils.distinct(items)
    ...
    def __str__(multiset):
        items_str = '{%s}' % ', '.join(
            '%r: %r' % pair for pair in sorted(iteritems(multiset._items_)))
...
class EntityMeta(type):
    ...
    def __init__(entity, name, bases, cls_dict):
        ...
        if not entity.__name__[:1].isupper():
            throw(ERDiagramError, 'Entity class name should start with a capital letter. Got: %s' %
                  entity.__name__)
        databases = set()
        for base_class in bases:
            if isinstance(base_class, EntityMeta):
                ...
                if database is None:
                    throw(ERDiagramError,
                          'Base Entity does not belong to any database')
        ...
        direct_bases = [c for c in entity.__bases__ if issubclass(
            c, Entity) and c.__name__ != 'Entity']
        ...
        if direct_bases:
            ...
            for base in direct_bases[1:]:
                if base._root_ is not root:
                    throw(ERDiagramError, 'Multiple inheritance graph must be diamond-like. '
        ...
        for name, attr in items_list(entity.__dict__):
            if name in base_attrs_dict:
                throw(ERDiagramError, "Name '%s' hides base attribute %s" %
                      (name, base_attrs_dict[name]))
            if not isinstance(attr, Attribute):
                continue
        ...
        entity._simple_keys_ = [key[0]
                                for key in entity._keys_ if len(key) == 1]
        entity._composite_keys_ = [
            key for key in entity._keys_ if len(key) > 1]
        ...
        for_expr = ast.GenExprFor(ast.AssName(
            iter_name, 'OP_ASSIGN'), ast.Name('.0'), [])
    ...
    def _initialize_bits_(entity):
        ...
        for attr in entity._attrs_:
            if attr.is_collection or attr.is_discriminator or attr.pk_offset is not None:
                bit = 0
    ...
    def _resolve_attr_types_(entity):
        ...
        for attr in entity._new_attrs_:
            ...
            if isinstance(py_type, basestring):
                ...
                if rentity is None:
                    throw(ERDiagramError,
                          'Entity definition %s was not found' % py_type)
    ...
    def _link_reverse_attrs_(entity):
        ...
        for attr in entity._new_attrs_:
            ...
            if isinstance(reverse, basestring):
                ...
                if attr2 is None:
                    throw(ERDiagramError, 'Reverse attribute %s.%s not found' %
                          (entity2.__name__, reverse))
            elif isinstance(reverse, Attribute):
                attr2 = reverse
                if attr2.entity is not entity2:
                    throw(ERDiagramError, 'Incorrect reverse attribute %s used in %s' % (
                        attr2, attr))
            elif reverse is not None:
                throw(ERDiagramError, "Value of 'reverse' option must be string. Got: %r" % type(
                    reverse))
            else:
                candidates1 = []
                candidates2 = []
                for attr2 in entity2._new_attrs_:
                    if attr2.py_type not in (entity, entity.__name__):
                        continue
            ...
            type2 = attr2.py_type
            if type2 != entity:
                throw(ERDiagramError, 'Inconsistent reverse attributes %s and %s' % (
                    attr, attr2))
            reverse2 = attr2.reverse
    ...
    def _get_pk_columns_(entity):
        if entity._pk_columns_ is not None:
    ...
    def __getitem__(entity, key):
        ...
        kwargs = {attr.name: value for attr,
                  value in izip(entity._pk_attrs_, key)}
        return entity._find_one_(kwargs)

    @cut_traceback
    def exists(entity, *args, **kwargs):
        if args:
            return entity._query_from_args_(args, kwargs, frame_depth=3).exists()
        try:
            obj = entity._find_one_(kwargs)
    ...
    def get(entity, *args, **kwargs):
        if args:
            return entity._query_from_args_(args, kwargs, frame_depth=3).get()
        try:
    ...
    def get_for_update(entity, *args, **kwargs):
        ...
        if args:
            return entity._query_from_args_(args, kwargs, frame_depth=3).for_update(nowait).get()
        try:
            # can throw MultipleObjectsFoundError
            return entity._find_one_(kwargs, True, nowait)
        except ObjectNotFound:
            return None
    ...
    def get_by_sql(entity, sql, globals=None, locals=None):
        # can throw MultipleObjectsFoundError
        objects = entity._find_by_sql_(1, sql, globals, locals, frame_depth=3)
        if not objects:
            return None
    ...
    def select_random(entity, limit):
        ...
        for i in xrange(5):
            ...
            if not ids:
                continue
            sql, adapter, attr_offsets = entity._construct_batchload_sql_(
    ...
    def _find_one_(entity, kwargs, for_update=False, nowait=False):
        if entity._database_.schema is None:
            throw(ERDiagramError, 'Mapping is not generated for entity %r' %
                  entity.__name__)
        pkval, avdict = entity._normalize_args_(kwargs, False)
        for attr in avdict:
            if attr.is_collection:
                throw(
                    TypeError, 'Collection attribute %s cannot be specified as search criteria' % attr)
        obj, unique = entity._find_in_cache_(pkval, avdict, for_update)
        if obj is None:
            obj = entity._find_in_db_(avdict, unique, for_update, nowait)
        if obj is None:
    ...
    def _find_in_db_(entity, avdict, unique=False, for_update=False, nowait=False):
        database = entity._database_
        query_attrs = {attr: value is None for attr,
                       value in iteritems(avdict)}
        limit = 2 if not unique else None
        sql, adapter, attr_offsets = entity._construct_sql_(
            query_attrs, False, limit, for_update, nowait)
        arguments = adapter(avdict)
        if for_update:
            database._get_cache().immediate = True
        cursor = database._exec_sql(sql, arguments)
        objects = entity._fetch_objects(
            cursor, attr_offsets, 1, for_update, avdict)
    ...
    def _construct_discriminator_criteria_(entity, alias=None):
        ...
        discr_values = [['VALUE', cls._discriminator_]
    ...
    def _construct_sql_(entity, query_attrs, order_by_pk=False, limit=None, for_update=False, nowait=False):
        ...
        for attr, attr_is_none in sorted_query_attrs:
            if not attr.reverse:
                if attr_is_none:
                    where_list.append(
                        ['IS_NULL', ['COLUMN', None, attr.column]])
                else:
                    if len(attr.converters) > 1:
                        throw(NotImplementedError)
                    converter = attr.converters[0]
                    where_list.append([converter.EQ, ['COLUMN', None, attr.column], [
        ...
        if not for_update:
            ...
            sql_ast = ['SELECT_FOR_UPDATE', bool(
                nowait), select_list, from_list, where_list]
        if order_by_pk:
            sql_ast.append(['ORDER_BY'] + [['COLUMN', None, column]
                                           for column in entity._pk_columns_])
        if limit is not None:
            sql_ast.append(['LIMIT', ['VALUE', limit]])
        database = entity._database_
        sql, adapter = database._ast2sql(sql_ast)
        cached_sql = sql, adapter, attr_offsets
    ...
    def _fetch_objects(entity, cursor, attr_offsets, max_fetch_count=None, for_update=False, used_attrs=()):
        ...
        if max_fetch_count is not None:
            ...
            if len(rows) == max_fetch_count + 1:
                if max_fetch_count == 1:
                    throw(MultipleObjectsFoundError,
        ...
        if attr_offsets is None:
            objects = [entity._get_by_raw_pkval_(
                row, for_update) for row in rows]
            entity._load_many_(objects)
        else:
            for row in rows:
                real_entity_subclass, pkval, avdict = entity._parse_row_(
                    row, attr_offsets)
                ...
                objects.append(obj)
        if used_attrs:
            entity._set_rbits(objects, used_attrs)
        return objects
    ...
    def _parse_row_(entity, row, attr_offsets):
        ...
        if not discr_attr:
            ...
            discr_value = discr_attr.validate(
                row[discr_offset], None, entity, from_db=True)
        ...
        for attr in real_entity_subclass._attrs_:
            ...
            if offsets is None or attr.is_discriminator:
                continue
            avdict[attr] = attr.parse_value(row, offsets)
        if not entity._pk_is_composite_:
            pkval = avdict.pop(entity._pk_attrs_[0], None)
        else:
            pkval = tuple(avdict.pop(attr, None) for attr in entity._pk_attrs_)
    ...
    def _query_from_args_(entity, args, kwargs, frame_depth):
        if not args and not kwargs:
            return entity._select_all()
        func, globals, locals = get_globals_and_locals(
            args, kwargs, frame_depth + 1)
        ...
        for_expr = ast.GenExprFor(ast.AssName(
            name, 'OP_ASSIGN'), ast.Name('.0'), [if_expr])
    ...
    def _get_from_identity_map_(entity, pkval, status, for_update=False, undo_funcs=None, obj_to_init=None):
        ...
        elif obj.__class__ is entity:
            pass
        elif issubclass(obj.__class__, entity):
            pass
    ...
    def _get_by_raw_pkval_(entity, raw_pkval, for_update=False, from_db=True):
        ...
        for attr in entity._pk_attrs_:
            if attr.column is not None:
                ...
                i += 1
                if not attr.reverse:
                    val = attr.validate(val, None, entity, from_db=from_db)
                else:
        ...
        for attr in entity._attrs_:
            if not attr.reverse:
                def fget(wrapper, attr=attr):
                    ...
                    items = [x for x in (attr.__get__(item)
                                         for item in wrapper) if x is not None]
                ...
                def fget(wrapper, attr=attr):
                    ...
                    cache.collection_statistics.setdefault(
                        attr, attr.nplus1_threshold)
                    attrnames = wrapper._attrnames_ + (attr.name,)
    ...
    def describe(entity):
        ...
        if entity._pk_is_composite_:
            result.append('PrimaryKey(%s)' % ', '.join(
                attr.name for attr in entity._pk_attrs_))
        return '\n    '.join(result)
    ...
    def _get_attrs_(entity, only=None, exclude=None, with_collections=False, with_lazy=False):
        if only and not isinstance(only, basestring):
            only = tuple(only)
        if exclude and not isinstance(exclude, basestring):
        ...
        if not attrs:
            ...
            if exclude:
                ...
                for attrname in exclude:
                    if attrname not in entity._adict_:
                        throw(AttributeError,
                              'Entity %s does not have attriute %s' % (entity.__name__, attrname))
...
statuses = {'created', 'cancelled', 'loaded', 'modified',
            'inserted', 'updated', 'marked_to_delete', 'deleted'}
...
class Entity(with_metaclass(EntityMeta)):
    ...
    def __reduce__(obj):
        ...
        if obj._status_ in ('created', 'modified'):
            throw(
        ...
        with cache.flush_disabled():
            ...
            try:
                entity._get_from_identity_map_(
                    pkval, 'created', undo_funcs=undo_funcs, obj_to_init=obj)
                for attr, val in iteritems(avdict):
                    if attr.pk_offset is not None:
                        continue
                    elif not attr.is_collection:
                ...
                raise
        if pkval is not None:
            cache_indexes[entity._pk_attrs_][pkval] = obj
        for key, vals in iteritems(indexes_update):
            cache_indexes[key][vals] = obj
        objects_to_save = cache.objects_to_save
        obj._save_pos_ = len(objects_to_save)
        objects_to_save.append(obj)
        cache.modified = True
    ...
    def _get_raw_pkval_(obj):
        ...
        if not obj._pk_is_composite_:
            if not obj._pk_attrs_[0].reverse:
                return (pkval,)
            else:
    ...
    def __repr__(obj):
        pkval = obj._pkval_
        if pkval is None:
            return '%s[new:%d]' % (obj.__class__.__name__, obj._newid_)
        if obj._pk_is_composite_:
            pkval = ','.join(imap(repr, pkval))
        else:
            pkval = repr(pkval)
    ...
    def _load_(obj):
        ...
        if cache is not database._get_cache():
            throw(TransactionError,
                  "Object %s doesn't belong to current transaction" % safe_repr(obj))
        seeds = cache.seeds[entity._pk_attrs_]
        max_batch_size = database.provider.max_params_count // len(
            entity._pk_columns_)
        objects = [obj]
    ...
    def load(obj, *attrs):
        ...
        if cache is not database._get_cache():
            throw(TransactionError,
                  "Object %s doesn't belong to current transaction" % safe_repr(obj))
        if obj._status_ in created_or_deleted_statuses:
            return
        if not attrs:
            ...
            for arg in args:
                if isinstance(arg, basestring):
                    ...
                    if attr is None:
                        ...
                        throw(
                            AttributeError, 'Object %s does not have attribute %r' % (obj, arg))
                elif isinstance(arg, Attribute):
                    attr = arg
                ...
                if entity._bits_[attr] and attr not in obj._vals_:
                    attrs.add(attr)
            ...
            try:
                for attr in obj._attrs_:
                    ...
                    if not attr.is_collection:
                        if not reverse.is_collection:
                            val = get_val(
                                attr) if attr in obj._vals_ else attr.load(obj)
                            if val is None:
                                continue
                            if attr.cascade_delete:
                                ...
                                throw(ConstraintError, "Cannot delete object %s, because it has associated %s, "
                                      "and 'cascade_delete' option of %s is not set"
                                      % (obj, attr.name, attr))
                        elif isinstance(reverse, Set):
                            if attr not in obj._vals_:
                                continue
                            val = get_val(attr)
                            if val is None:
                                continue
        ...
        with cache.flush_disabled():
            ...
            if avdict:
                ...
                if not collection_avdict:
                    for attr in avdict:
                        if attr.reverse or attr.is_part_of_unique_index:
                            break
                    else:
    ...
    def _keyargs_to_avdicts_(obj, kwargs):
        ...
        for name, new_val in kwargs.items():
            ...
            if attr.is_collection:
                ...
                throw(
                    TypeError, 'Cannot change value of primary key attribute %s' % attr.name)
    ...
    def _construct_optimistic_criteria_(obj):
        ...
        for attr in obj._attrs_with_bit_(obj._attrs_with_columns_, obj._rbits_):
            converters = attr.converters
            assert converters
            if not (attr.optimistic and converters[0].optimistic):
                continue
            ...
            if dbval is None:
                ...
                optimistic_operations.extend(
                    converter.EQ for converter in converters)
    ...
    def _save_principal_objects_(obj, dependent_objects):
        if dependent_objects is None:
            ...
            throw(UnresolvableCyclicDependency,
                  'Cannot save cyclic chain: ' + chain)
        dependent_objects.append(obj)
        status = obj._status_
        if status == 'created':
            attrs = obj._attrs_with_columns_
        elif status == 'modified':
            attrs = obj._attrs_with_bit_(obj._attrs_with_columns_, obj._wbits_)
        else:
            assert False  # pragma: no cover
        for attr in attrs:
            if not attr.reverse:
    ...
    def _save_updated_(obj):
        ...
        if update_columns:
            ...
            query_key = tuple(update_columns), tuple(
                optimistic_columns), tuple(optimistic_ops)
            database = obj._database_
            ...
            if cached_sql is None:
                ...
                update_params = [['PARAM', (i, None, None), converter]
                ...
                sql_ast = ['UPDATE', obj._table_, list(
                    izip(update_columns, update_params)), where_list]
                sql, adapter = database._ast2sql(sql_ast)
    ...
    def _save_deleted_(obj):
        ...
        if cached_sql is None:
            where_list = ['WHERE']
            params_count = populate_criteria_list(
                where_list, obj._pk_columns_, obj._pk_converters_, repeat('EQ'))
            if optimistic_columns:
                populate_criteria_list(
                    where_list, optimistic_columns, optimistic_converters, optimistic_ops, params_count, optimistic=True)
    ...
    def flush(obj):
        ...
        with cache.flush_disabled():
            ...
            # TODO: add to documentation that flush is disabled inside
            # before_xxx hooks
            obj._save_()
        cache.call_after_save_hooks()
    ...
    def to_dict(obj, only=None, exclude=None, with_collections=False, with_lazy=False, related_objects=False):
        if obj._session_cache_.modified:
            obj._session_cache_.flush()
...
def string2ast(s):
    ...
    if not isinstance(stmt_node, ast.Stmt) or len(stmt_node.nodes) != 1:
        throw(TypeError)
    discard_node = stmt_node.nodes[0]
    if not isinstance(discard_node, ast.Discard):
        throw(TypeError)
...
def get_globals_and_locals(args, kwargs, frame_depth, from_generator=False):
    ...
    if args_len > 1:
        ...
        if len(args) > 3:
            throw(TypeError, 'Excess positional argument%s: %s'
    ...
    if kwargs:
        throw(TypeError, 'Keyword arguments cannot be specified together with positional arguments')
    ...
    for key, code in iteritems(extractors):
        ...
        try:
            ...
            if unsupported:
                ...
                if src == '.0':
                    throw(TypeError, 'Cannot iterate over non-entity object')
                throw(TypeError, 'Expression `%s` has unsupported type %r' %
                      (src, typename))
            vartypes[key] = get_normalized_type_of(value)
        vars[key] = value
...
class Query(object):
    ...
    def __init__(query, code_key, tree, globals, locals, cells=None, left_join=False):
        ...
        if database is None:
            throw(TranslationError, 'Entity %s is not mapped to a database' %
                  origin.__name__)
        if database.schema is None:
            throw(ERDiagramError, 'Mapping is not generated for entity %r' %
                  origin.__name__)
        ...
        query._vars = vars
        query._key = pretranslator_key, tuple(
            vartypes[name] for name in varnames), left_join
        ...
        if translator is None:
            ...
            translator = translator_cls(
                tree, extractors, vartypes, left_join=left_join)
            name_path = translator.can_be_optimized()
            if name_path:
                tree = unpickle_ast(pickled_tree)  # tree = deepcopy(tree)
                try:
                    translator = translator_cls(
                        tree, extractors, vartypes, left_join=True, optimize=name_path)
    ...
    def _construct_sql_and_arguments(query, range=None, aggr_func_name=None):
        ...
        if isinstance(expr_type, EntityMeta) and query._attrs_to_prefetch_dict:
            attrs_to_prefetch = tuple(
                sorted(query._attrs_to_prefetch_dict.get(expr_type, ())))
        else:
            attrs_to_prefetch = ()
        ...
        if query._translator.query_result_is_cacheable:
            ...
            if arguments_type is tuple:
                arguments_key = arguments
            elif arguments_type is dict:
                arguments_key = tuple(sorted(iteritems(arguments)))
            try:
                hash(arguments_key)
            except:
                query_key = None  # arguments are unhashable
            else:
                query_key = sql_key + (arguments_key)
        else:
            query_key = None
    ...
    def prefetch(query, *args):
        ...
        for arg in args:
            if isinstance(arg, EntityMeta):
                entity = arg
                if query._database is not entity._database_:
                    throw(TypeError,
                ...
                throw(TypeError, 'Argument of prefetch() query method must be entity class or attribute. '
                      'Got: %r' % arg)
        return query
    ...
    def _do_prefetch(query, result):
        ...
        for obj in object_list:
            entity = obj.__class__
            if obj in cache.seeds[entity._pk_attrs_]:
                obj._load_()
            ...
            if all_attrs_to_prefetch is None:
                ...
                for attr in obj._attrs_:
                    if attr.is_collection:
                        if attr in attrs_to_prefetch:
                            append(attr)
                    elif attr.is_relation:
                        if attr in attrs_to_prefetch or attr.py_type in entities_to_prefetch:
                            append(attr)
                    elif attr.lazy:
                        if attr in attrs_to_prefetch:
                            append(attr)
    ...
    def first(query):
        ...
        if translator.order:
            ...
            query = query.order_by(
                *[i + 1 for i in xrange(len(query._translator.expr_type))])
        else:
    ...
    def order_by(query, *args):
        if not args:
            throw(TypeError, 'order_by() method requires at least one argument')
        if args[0] is None:
            if len(args) > 1:
                throw(
                    TypeError, 'When first argument of order_by() method is None, it must be the only argument')
            tup = ((),)
        ...
        for arg in args:
            if isinstance(arg, int_types):
                ...
                throw(
                    TypeError, "order_by() method receive an argument of invalid type: %r" % arg)
        ...
        if new_translator is None:
            if numbers:
                new_translator = query._translator.order_by_numbers(args)
            else:
                new_translator = query._translator.order_by_attributes(args)
            query._database._translator_cache[new_key] = new_translator
        return query._clone(_key=new_key, _filters=new_filters, _translator=new_translator)
    ...
    def _process_lambda(query, func, globals, locals, order_by):
        ...
        if extractors:
            ...
            new_query_vars, vartypes, sorted_vartypes = query._vars, {}, ()

        new_key = query._key + \
            (('order_by' if order_by else 'filter',
              pretranslator_key, sorted_vartypes),)
        new_filters = query._filters + \
            ((order_by, func_ast, argnames, extractors, vartypes),)
        new_translator = query._database._translator_cache.get(new_key)
        if new_translator is None:
            prev_optimized = prev_translator.optimize
            new_translator = prev_translator.apply_lambda(
                filter_num, order_by, func_ast, argnames, extractors, vartypes)
            if not prev_optimized:
                name_path = new_translator.can_be_optimized()
                if name_path:
                    # tree = deepcopy(tree)
                    tree = unpickle_ast(prev_translator.pickled_tree)
    ...
    def _reapply_filters(query, translator):
        for i, tup in enumerate(query._filters):
            if not tup:
                ...
                translator = translator.apply_lambda(
                    i + 1, order_by, func_ast, argnames, extractors, vartypes)
        return translator
        ...
        if isinstance(key, slice):
            ...
            if stop is None:
                if not start:
                    ...
                    throw(
                        TypeError, "Parameter 'stop' of slice object should be specified")
        else:
    ...
    def for_update(query, nowait=False):
        provider = query._database.provider
        if nowait and not provider.select_for_update_nowait_syntax:
            throw(TranslationError,
...
class QueryResult(list):
    ...
    def show(result, width=None):
        ...
        if isinstance(expr_type, EntityMeta):
            ...
            rows = [tuple(to_str(value) for value in row_maker(obj))
                    for obj in result]
        ...
        for col_num, colname in enumerate(col_names):
            if not rows:
                ...
                max_len = max(len(colname), max(
                    len(row[col_num]) for row in rows))
        ...
        print(strjoin('|', (strcut(colname, width_dict[
              i]) for i, colname in enumerate(col_names))))
        print(strjoin('+', ('-' * width_dict[i]
                            for i in xrange(len(col_names)))))
        for row in rows:
            print(
                strjoin('|', (strcut(item, width_dict[i]) for i, item in enumerate(row))))
...
special_functions = {itertools.count,
                     utils.count, count, random, raw_sql, getattr}
const_functions = {buffer, Decimal, datetime.datetime,
