...
class Database(object):
    ...
    def generate_mapping(database, filename=None, check_tables=True, create_tables=False):
        ...
        def fix_obsolete_table_name(table_name):
            if isinstance(table_name, tuple):
                return
            if not getattr(table_name, 'autogenerated', False):
                return
            fixed_obsolete_name = obsolete_name = obsolete(table_name)
            i = 0
            while fixed_obsolete_name in obsolete_table_names:
                i += 1
                fixed_obsolete_name = '%s_%d' % (obsolete_name, i)
            if fixed_obsolete_name != obsolete_name:
                table_name.obsolete_name = fixed_obsolete_name

        for entity in entities:
            table_name = fix_obsolete_table_name(entity.meta.get_root().table.name)
            ...
            for attr in entity._attrs_:
                ...
                if attr.is_collection and attr.reverse.is_collection:
                    m2m_table_name = fix_obsolete_table_name(attr.meta.m2m_table.name)
                    attr.table = m2m_table_name
