...
class TestManyToManyNonComposite(unittest.TestCase):
    ...
    def q(self, s):
        return s.replace('`', '"') if db.provider.dialect in ('PostgreSQL', 'Oracle') else s

    def test_1(self):
        schema = db.schema
        # m2m_table_name = 'Group_Subject'
        m2m_table_name = 'group_subjects'  # NEW_NAME
    ...
    def test_2(self):
        ...
        with db_session:
            # db_subjects = db.select('subject from Group_Subject where "group" = 101')  # NEW_NAME
            db_subjects = db.select(self.q('`subject_name` from `group_subjects` where `group_number` = 101'))
    ...
    def test_10(self):
        ...
        with db_session:
            self.assertEqual(db.last_sql, last_sql)  # assert no DELETE statement on commit
            # db_subjects = db.select('subject from Group_Subject where "group" = 102')  # NEW_NAME
            db_subjects = db.select(self.q('`subject_name` from `group_subjects` where `group_number` = 102'))
