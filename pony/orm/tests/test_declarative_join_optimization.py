...
class TestM2MOptimization(unittest.TestCase):
    ...
    def test1(self):
        ...
        self.assertEqual(Course._table_ not in flatten(
    ...
    def test4(self):
        ...
        self.assertEqual(Group._table_ not in flatten(
            q._translator.conditions), True)

    def test5(self):
        q = select(s for s in Student if s.group.number ==
                   1 or s.group.major == '1')
        self.assertEqual(Group._table_ in flatten(
            q._translator.subquery.from_ast), True)
    ...
    def test7(self):
        q = select(s for s in Student if sum(
            c.credits for c in Course if s.group.dept == c.dept) > 10)
        objects = q[:]
