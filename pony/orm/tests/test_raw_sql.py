...
class TestRawSQL(unittest.TestCase):
    ...
    def test_1(self):
        ...
        persons = select(p for p in Person if raw_sql(
            'abs("p"."age") > 25'))[:]
    ...
    @db_session
    def test_10(self):
        # raw_sql does not know result type and cannot appy correct type
        # converter automatically
        dates = select(raw_sql('(p.dob)')
    ...
    def test_11(self):
        ...
        dates = select(raw_sql('(p.dob)', result_type=date)
                       for p in Person).order_by(lambda: p.id)[:]
        self.assertEqual(
            dates, [date(1985, 1, 1), date(1983, 5, 20), date(1995, 2, 15)])
    ...
    def test_13(self):
        # raw_sql in filter()
        x = 25
        persons = select(p for p in Person).filter(
            lambda p: p.age > raw_sql('$x'))[:]
        self.assertEqual(set(persons), {Person[1], Person[2]})
    ...
    def test_15(self):
        ...
        x = '123'
        y = 'John'
        persons = Person.select(lambda p: raw_sql(
            "UPPER(p.name) || $x") == raw_sql("UPPER($y || '123')"))[:]
        self.assertEqual(set(persons), {Person[1]})

    @db_session
    def test_16(self):
        # the same param name can be used several times with different types &
        # values
        x = 10
        y = 31
        ...
        x = date(1980, 1, 1)
        y = 'j'
        q = q.filter(lambda p: p.dob >
                     x and p.name.startswith(raw_sql('UPPER($y)')))
        persons = q[:]
        self.assertEqual(set(persons), {Person[1]})
    ...
    def test_17(self):
        ...
        persons = Person.select().order_by(
            lambda p: raw_sql('SUBSTR(p.dob, $x)'))[:]
    ...
    def test_21(self):
        x = None
        persons = select(p for p in Person if p.id ==
                         1 and raw_sql('$x') is None)[:]
