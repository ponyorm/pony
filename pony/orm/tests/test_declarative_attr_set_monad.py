...
class TestAttrSetMonad(unittest.TestCase):
    ...
    def test4(self):
        groups = select(g for g in Group if max(
            g.students.marks.value) <= 2)[:]
        self.assertEqual(groups, [Group[42]])

    def test5(self):
        students = select(s for s in Student if len(
            s.marks.subject.name) > 5)[:]
    ...
    def test8(self):
        students = set(select(s for s in Student if s.group in (
            g for g in Group if g.department == 101)))
        self.assertEqual(students, {Student[1], Student[2], Student[3]})

    def test9(self):
        students = set(select(s for s in Student if s.group not in (
            g for g in Group if g.department == 101)))
    ...
    def test11(self):
        students = set(select(g for g in Group if len(
            g.subjects.groups.subjects) > 1))
    ...
    def test18(self):
        groups = set(
            select(g for g in Group if 100 not in g.students.scholarship))
        self.assertEqual(groups, {Group[42], Group[43], Group[44]})

    def test19(self):
        groups = set(
            select(g for g in Group if not not not 100 not in g.students.scholarship))
        self.assertEqual(groups, {Group[41]})

    def test20(self):
        groups = set(select(g for g in Group if exists(
            s for s in Student if s.group == g and s.scholarship == 500)))
        self.assertEqual(groups, {Group[41], Group[42]})
    ...
    def test21b(self):
        groups = set(
            select(g for g in Group if not not not g.department is None))
        self.assertEqual(groups, {Group[41], Group[42], Group[43], Group[44]})

    def test22(self):
        groups = set(select(g for g in Group if 700 in (
            s.scholarship for s in Student if s.group == g)))
        self.assertEqual(groups, {Group[42]})

    def test23a(self):
        groups = set(
            select(g for g in Group if 700 not in g.students.scholarship))
        self.assertEqual(groups, {Group[41], Group[43], Group[44]})

    def test23b(self):
        groups = set(select(g for g in Group if 700 not in (
            s.scholarship for s in Student if s.group == g)))
        self.assertEqual(groups, {Group[41], Group[43], Group[44]})
    ...
    def test24(self):
        groups = set(
            select(g for g in Group for g2 in Group if g.students == g2.students))
