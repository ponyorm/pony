...
class TestFuncMonad(unittest.TestCase):
    ...
    def test_minmax5(self):
        ...
        try:
            ...
            self.assertTrue(PY2 and e.args[
                            0] == "The bytestring '\\x80' contains non-ascii symbols. Try to pass unicode string instead")
        else:
    ...
    def test_minmax6(self):
        ...
        try:
            result = set(
                select(s for s in Student if min(s.name, x, "CC") == "CC"))
        except TypeError as e:
            self.assertTrue(PY2 and e.args[
                            0] == "The bytestring '\\x80' contains non-ascii symbols. Try to pass unicode string instead")
        else:
    ...
    def test_date_func2(self):
        result = set(
            select(s for s in Student if s.dob >= date(1983, 'three', 3)))
    ...
    def test_datetime_func1(self):
        result = set(
            select(s for s in Student if s.last_visit >= date(2011, 3, 3)))
        self.assertEqual(result, {Student[3], Student[4], Student[5]})

    def test_datetime_func2(self):
        result = set(
            select(s for s in Student if s.last_visit >= datetime(2011, 3, 3)))
        self.assertEqual(result, {Student[3], Student[4], Student[5]})

    def test_datetime_func3(self):
        result = set(select(s for s in Student if s.last_visit >=
                            datetime(2011, 3, 3, 13, 13, 13)))
        self.assertEqual(result, {Student[3], Student[4], Student[5]})
    ...
    def test_decimal_func(self):
        result = set(
            select(s for s in Student if s.scholarship >= Decimal("303.3")))
        self.assertEqual(result, {Student[3], Student[4], Student[5]})

    def test_concat_1(self):
        result = set(select(concat(s.name, ':', s.dob.year,
                                   ':', s.scholarship) for s in Student))
        self.assertEqual(result, {'AA:1981:0', 'BB:1982:202.2',
                                  'CC:1983:303.3', 'DD:1984:404.4', 'EE:1985:505.5'})
