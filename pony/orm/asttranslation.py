...
class ASTTranslator(object):
    ...
    def dispatch(translator, node):
        ...
        try:
            pre_method = translator.pre_methods[cls]
        except KeyError:
            pre_method = getattr(translator, 'pre' +
                                 cls.__name__, translator.default_pre)
        ...
        try:
            post_method = translator.post_methods[cls]
        except KeyError:
            post_method = getattr(translator, 'post' +
                                  cls.__name__, translator.default_post)
    ...
    def __init__(translator, tree, globals, locals,
        ...
        for node in externals.copy():
            if isinstance(node, nonexternalizable_types) \
                ...
                externals.update(node for node in node.getChildNodes(
...
def create_extractors(code_key, tree, filter_num, globals, locals,
    ...
    if getattr_extractors:
        getattr_attrname_values = tuple(
            eval(code, globals, locals) for src, code in getattr_extractors)
    ...
    if not result:
        ...
        for node in pretranslator.getattr_nodes:
            if node in pretranslator.externals:
                ...
                throw(
                    TypeError, '`%s` should be either external expression or constant.' % ast2src(node))
        ...
        getattr_attrname_values = tuple(
            val for key, val in sorted(getattr_attrname_values.items()))
        extractors_key = (code_key, filter_num, getattr_attrname_values)
        result = extractors_cache[
            extractors_key] = extractors, varnames, tree, extractors_key
    return result
