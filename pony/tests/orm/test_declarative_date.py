...
class TestDate(unittest.TestCase):
    ...
    def test_datetime_year(self):
        result = select(e for e in Entity1 if e.c.year > 2009).all()
        self.assertEquals(len(result), 2)
    def test_datetime_month(self):
        result = select(e for e in Entity1 if e.c.month == 10).all()
        self.assertEquals(len(result), 2)
    def test_datetime_day(self):
        result = select(e for e in Entity1 if e.c.day == 22).all()
        self.assertEquals(len(result), 1)
    def test_datetime_hour(self):
        result = select(e for e in Entity1 if e.c.hour == 10).all()
        self.assertEquals(len(result), 3)
    def test_datetime_minute(self):
        result = select(e for e in Entity1 if e.c.minute == 20).all()
        self.assertEquals(len(result), 2)
    def test_datetime_second(self):
        result = select(e for e in Entity1 if e.c.second == 30).all()
        self.assertEquals(len(result), 1)
