...
class TestSQLTranslatorExceptions(unittest.TestCase):
    ...
    def test40(self):
        fetch_all(s for s in Student if sum(c.name for c in s.courses) > 10)
    @raises_exception(TypeError, "Function 'sum' expects query or items of numeric type, got 'unicode' in sum(c.name for c in s.courses)")
    def test41(self):
        fetch_all(s for s in Student if sum(c.name for c in s.courses) > 10)
    ...
    def test42(self):
        fetch_all(s for s in Student if avg(c.name for c in s.courses) > 10 and len(s.courses) > 1)
    @raises_exception(TypeError, "strip() takes at most 1 argument (3 given)")
    def test43(self):
        fetch_all(s for s in Student if s.name.strip(1, 2, 3))
    @raises_exception(TypeError, "len() takes exactly 1 argument (2 given)")
    def test44(self):
        fetch_all(s for s in Student if len(1, 2) == 3)
    @raises_exception(NotImplementedError, "Group[101].students")
    def test45(self):
        fetch_all(s for s in Student if s in Group[101].students)
    ...
    def test46(self):
        fetch_all(g for g in Group if sum(s for s in Student if s.group == g) > 1)
    ...
    def test47(self):
        fetch_all(g for g in Group if avg(s for s in Student if s.group == g) > 1)
    ...
    def test48(self):
        fetch_all(g for g in Group if min(s for s in Student if s.group == g) > 1)
    ...
    def test49(self):
        fetch_all(g for g in Group if max(s for s in Student if s.group == g) > 1)
    @raises_exception(TypeError, "Incomparable types 'Decimal' and 'bool' in expression: s.scholarship == (True or False and not True)")
    def test50(self):
        fetch_all(s for s in Student if s.scholarship == (True or False and not True))
    @raises_exception(NotImplementedError, "+3")
    def test51(self):
        fetch_all(s for s in Student if s.name > +3)
    @raises_exception(NotImplementedError, "{'a':'b'}")
    def test52(self):
        fetch_all(s for s in Student if s.name == {'a' : 'b'})
    @raises_exception(NotImplementedError, "a ^ 2")
    def test53(self):
        a = 1
        fetch_all(s for s in Student if s.name > a ^ 2)
    @raises_exception(NotImplementedError, "a | 2")
    def test54(self):
        a = 1
        fetch_all(s for s in Student if s.name > a | 2)
    ...
    def test58(self):
        a = 1
        fetch_all(s for s in Student if s.name > a * 2 % 4)
    @raises_exception(NotImplementedError, "~a")
    def test59(self):
        a = 1
        fetch_all(s for s in Student if s.name > ~a)
    @raises_exception(TypeError, "Incomparable types 'unicode' and 'int' in expression: s.name > 1 / a - 3")
    def test60(self):
        a = 1
        fetch_all(s for s in Student if s.name > 1 / a - 3)
    ...
    def test62(self):
        fetch_all(s for s in Student if s.name == [1, (2,)])
