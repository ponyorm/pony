...
def get_lambda_args(func):
    ...
    if type(func) is types.FunctionType:
        if hasattr(inspect, 'signature'):
            ...
            for p in inspect.signature(func).parameters.values():
                ...
                if p.kind == p.POSITIONAL_OR_KEYWORD:
                    ...
                    throw(
                        TypeError, 'Positional-only arguments like %s are not supported' % p.name)
                elif p.kind == p.KEYWORD_ONLY:
                    throw(
                        TypeError, 'Keyword-only arguments like %s are not supported' % p.name)
                else:
                    assert False
        else:
        ...
        names = func.argnames
        if func.kwargs:
            names, kwname = names[:-1], names[-1]
        else:
            kwname = None
        if func.varargs:
            names, argsname = names[:-1], names[-1]
        else:
            argsname = None
    ...
    if not is_absolute_path(code_filename):
        if code_filename.startswith('<') and code_filename.endswith('>'):
            ...
            raise EnvironmentError(
                'Unexpected module filename, which is not absolute file path: %r' % code_filename)
...
def tostring(x):
    ...
    if hasattr(x, 'makeelement'):
        return cElementTree.tostring(x)
    try:
        return str(x)
    except:
        pass
...
def strjoin(sep, strings, source_encoding='ascii', dest_encoding=None):
    ...
    for i, s in enumerate(strings):
        if isinstance(s, str):
            strings[i] = s.decode(
                source_encoding, 'replace').replace(u'\ufffd', '?')
