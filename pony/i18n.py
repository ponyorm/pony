...
def read_phrases(lines):
   ...
   for lineno, line in izip(count(1), lines):
       if line.isspace(): continue
           ...
           if kstr is None: raise ParseError("Translation string found but key string was expected in line %d" % lineno)
...
def check_params(params_list, lineno, lstr, lineno2, langkey):    
    ...
    if len(params_list) != len(params_list2):
        raise ParseError("Parameters count in line %d doesn't match with line %d" % (lineno, lineno2))
    params_list.sort()
    params_list2.sort()
    for a, b in zip(params_list, params_list2):
        if a != b:
            raise ParseError("Unknown parameter in line %d: %s (translation for %s)" % (lineno2, b, langkey))
